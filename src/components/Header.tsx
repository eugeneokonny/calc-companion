      ...
      <div className="container max-w-7xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2">
            ...
            label,
            icon: Icon
          }) => <Link key={path} to={path} className={`px-3 py-1.5 rounded text-xs font-mono transition-colors flex items-center gap-1.5 ${location.pathname === path ? "bg-primary/10 text-primary border border-primary/20" : "bg-muted/50 text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
                <Icon className="h-3.5 w-3.5" />
                {label}
              </Link>)}
            <span className="px-2 py-1 rounded text-primary font-mono text-xs ml-2 bg-secondary">
              v1.0
            </span>
          </nav>
        </div>
